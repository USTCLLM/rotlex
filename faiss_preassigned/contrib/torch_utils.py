...
def handle_torch_Index(the_class):
    ...
    def torch_replacement_search_preassigned(self, x, k, Iq, Dq, *, D=None, I=None, stats=None):
        if type(x) is np.ndarray:
            # forward to faiss __init__.py base method
        ...
        if x.is_cuda:
            ...
            # On the GPU, use proper stream ordering
            with using_stream(self.getResources()):
                self.search_preassigned_c(n, x_ptr, k, Iq_ptr, Dq_ptr, D_ptr, I_ptr, False, stats=stats)
        else:
            # CPU torch
            self.search_preassigned_c(n, x_ptr, k, Iq_ptr, Dq_ptr, D_ptr, I_ptr, False, stats=stats)
