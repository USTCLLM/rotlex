...
void IndexShardsIVF::add_with_ids( // mod here
        idx_t n,
        const component_t* x,
        const idx_t* xids) {
...
void IndexShardsIVF::search(
    ...
    // IVFSearchParameters params_shard;
    // params_shard.nprobe = nprobe;
...
void IndexShardsIVF::search_preassigned(
        idx_t n,
        const component_t* x,
        idx_t k,
        const idx_t* keys,
    ...
    // coarse quantization (TODO: support tiling with search_precomputed)
    // fprintf(stderr, "IT IS OK BEFORE\n");
    // fprintf(stderr, "%ld %ld\n", n, nprobe);

    std::vector<distance_t> Dq(coarse_dis, coarse_dis + n * nprobe);
    std::vector<idx_t> Iq(keys, keys + n * nprobe);
    // fprintf(stderr, "IT IS OK AFTER\n");
    // quantizer->search(n, x, nprobe, Dq.data(), Iq.data());
