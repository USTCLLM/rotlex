...
idx_t IVFBase::addVectors(
    ...
    if(assign){
        //FAISS_ASSERT(!useResidual_);
        if(useResidual_){
            idx_t n = vecs.getSize(0);
            std::vector<float> vresiduals(n * dim_);
            std::vector<float> vvecs = vecs.copyToVector(stream);
            coarseQuantizer->compute_residual_n(n, vvecs.data(), vresiduals.data(), assign);
            residuals.copyFrom(vresiduals, stream);
        }
        ivfIndices.copyFrom(Tensor<idx_t, 2, true>(const_cast<idx_t*>(assign), {vecs.getSize(0), 1}), stream);
    }else{
