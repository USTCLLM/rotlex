...
void GpuIndexIVF::verifyIVFSettings_() const {
    ...
    if (is_trained) {
        ...
        // WWQ: for our case DO NOT check this
...
void GpuIndexIVF::addImpl_(idx_t n, const float* x, const idx_t* xids, const idx_t* assign) {
    ...
    baseIndex_->addVectors(quantizer, data, labels, assign);
...
void GpuIndexIVF::add_preassigned(idx_t n, const float* x, const idx_t* assign) {
    // Pass to add_preassigned_with_ids
    add_preassigned_with_ids(n, x, nullptr, assign);
}

void GpuIndexIVF::add_preassigned_with_ids(idx_t n, const float* x, const idx_t* ids, const idx_t* assign){
    ...
    addPaged_(n, x, ids ? ids : generatedIds.data(), assign);  
}
