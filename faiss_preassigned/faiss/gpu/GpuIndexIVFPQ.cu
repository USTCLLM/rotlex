...
void GpuIndexIVFPQ::trainResidualQuantizer_(idx_t n, const float* x, const idx_t* preassign) {
    ...
    if(!preassign){
        ...
        for(int i=0;i<n;++i){
            assign.emplace_back(preassign[i]);
        }
    }
    std::vector<float> residuals(n * d);
    quantizer->compute_residual_n(n, x, residuals.data(), assign.data());//pre compute centor
...
void GpuIndexIVFPQ::add_preassigned(
        idx_t n,
        const float* x,
        const idx_t* assign) {
    if(!index_){
    ...
    centroids.resize(d * nlist);
    size_t k_frozen = 0;
    std::vector<float> hassign(nlist);
    compute_centroids(
                    d,
                    nlist,
                    n,
    ...
    auto hostData = toHost<float, 2>(
                ...
                {n, this->d});
    //trainQuantizer_(n, hostData.data());
    trainResidualQuantizer_(n, hostData.data()); //, assign
    ...
    GpuIndexIVF::add_preassigned(n, x, assign); 
    // this->is_trained = true;
    // GpuIndexIVF::add(n, x); 
}

void GpuIndexIVFPQ::add_preassigned_with_ids(
        idx_t n,
        const float* x,
        const idx_t* ids,
        const idx_t* assign) {
    ...
    GpuIndexIVF::add_preassigned_with_ids(n, x, ids, assign);        
}
