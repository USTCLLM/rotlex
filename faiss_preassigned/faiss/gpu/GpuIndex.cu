...
void GpuIndex::addPaged_(idx_t n, const float* x, const idx_t* ids, const idx_t* assign) {
    if (n > 0) {
        ...
        if (!should_use_raft(config_) &&
            ...
            // FAISS_ASSERT(!assign);// WWQ!!!: not impletmented
            ...
            for (idx_t i = 0; i < n; i += tileSize) {
                ...
                addPage_(curNum, x + i * this->d, ids ? ids + i : nullptr, assign? assign + i:nullptr);
            }
        } else {
            addPage_(n, x, ids, assign);
        }
    }
}

void GpuIndex::addPage_(idx_t n, const float* x, const idx_t* ids, const idx_t* assign) {
    ...
    if (ids) {
        auto indices = toDeviceTemporary<idx_t, 1>(
                ...
                {n});
        if(assign){
            // auto ivfIndices = toDeviceTemporary<idx_t, 1>(
            //     resources_.get(),
            //     config_.device,
            //     const_cast<idx_t*>(assign),
            //     stream,
            //     {n});
            addImpl_(n, vecs.data(), indices.data(), assign);
        }else{
            addImpl_(n, vecs.data(), indices.data(), nullptr);
        }
    } else {
        FAISS_ASSERT(assign == nullptr);//All IVF's ids is not null. Only IVF need `assign`
        addImpl_(n, vecs.data(), nullptr, nullptr);
    }
}
